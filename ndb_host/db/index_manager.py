...
class OrbitDBManager:
    ...
    def add_relation(self, source, target, relation: str, weight: float | None = None) -> None:
    ...
    def get_metadata(self, record_id: int) -> dict[str, Any] | None:
    ...
    def update_vec(self, record_id: int, vector: np.ndarray | None = None,
                   metadata: dict[str, Any] | None = None) -> tuple[int, str | None]:
...
class CorpusManager:
    ...
    def _validate_paths(self) -> None:
        ...
        if not self.storage_path.exists() or not self.storage_path.is_dir():
            errors.append(f"Nova storage path missing: {self.storage_path}")
    ...
    def create_corpus(
        ...
        corpus_name: str,
        username: str,
        ndb_type: NDBMeta.Type = NDBMeta.Type.COSMOS,
        status: str = "active",
    ) -> None:
        """
        Create a new corpus.
        ...
        metadata = self.generate_corpus_metadata(
            corpus_name=corpus_name,
            created_by=username,
            status=status,
            ndb_type=ndb_type,
...
class SegmentManager:
    ...
    def _validate_paths(self) -> None:
        ...
        if not self.corpus_path.exists() or not self.corpus_path.is_dir():
            errors.append(f"Nova storage path missing: {self.corpus_path}")
        if errors:
            raise FileNotFoundError(" | ".join(errors))
    ...
    def _load_relations(
            ...
            src_col = source_column or next(
                (c for c in self.RELATION_SOURCE_COLS if c in segment_dataset.columns), None
            )
            tgt_col = target_column or next(
                (c for c in self.RELATION_TARGET_COLS if c in segment_dataset.columns), None
            )
            rel_col = relation_column or next(
                (c for c in self.RELATION_LABEL_COLS if c in segment_dataset.columns), None
            )
                    ...
                    for idx in range(segment_dataset.height):
                        try:
                            ...
                            if source is None or target is None:
                                continue
                            label = (
                                segment_dataset[rel_col][idx]
                                if rel_col and rel_col in segment_dataset.columns
                                else default_label
                            )
                            weight = None
