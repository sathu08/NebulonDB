...
async def create_segment(
    ...
    try:
        ...
        available_segment_list = segment_manager.get_available_segment_list()

        if available_segment_list and segment_name in available_segment_list.get("product_names", []):
        ...
        if not check_user_permission(current_user=current_user, required_role=UserRole.ADMIN_USER):
            logger.error(f"User '{current_user.username}' lacks required permission to create segment")
            return SegmentExistenceResponse(
                exists=True,
                corpus_name=corpus_name,
                segment_name=segment_name,
        ...
        segment_manager.create_segment(segment_name=segment_name, category=category_name)

        logger.info(f"Segment created successfully in {corpus_name}: {segment_name}")
        return SegmentExistenceResponse(
            exists=False,
            corpus_name=corpus_name,
            segment_name=segment_name,
            message=f"Segment {segment_name} created successfully in {corpus_name}"
        )
